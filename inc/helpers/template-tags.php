...
function anacours_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'anacours' ),
 ...
 * @return void
 */
function anacours_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'anacours' ),
 ...
 * @return bool|string
 */
function anacours_the_excerpt( $trim_character_count = 0 ) {
 ...
 */
function anacours_excerpt_more( $more = '' ) {

	if ( ! is_single() ) {
		$more = sprintf( '<a class="anacours-read-more text-white" href="%1$s"><button class="mt-3 btn btn-info">%2$s</button></a>',
			get_permalink( get_the_ID() ),
			__( 'Read more', 'anacours' )
		);
	}
...
function anacours_pagination() {
	...
	printf( '<nav class="anacours-pagination clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}
